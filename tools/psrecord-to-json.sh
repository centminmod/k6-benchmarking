...
converter() {
  ...
  if [[ "$mode" = 'json' ]]; then
    ...
    # curl -i -w 'HTTP %{http_version} %{http_code} d:%{size_download} u:%{size_upload} %header{x-request-id} %header{x-influxdb-error}' -sX POST http://localhost:8186/query --data-urlencode "q=CREATE DATABASE psrecord"
    # curl -i -w 'HTTP %{http_version} %{http_code} d:%{size_download} u:%{size_upload} %header{x-request-id} %header{x-influxdb-error}' -sX POST 'http://localhost:8186/write?db=psrecord' --data-binary @cpuload.txt
    # curl -i -w 'HTTP %{http_version} %{http_code} d:%{size_download} u:%{size_upload} %header{x-request-id} %header{x-influxdb-error}' -sX POST 'http://localhost:8186/write?db=psrecord' --data-binary @realmem.txt
    # curl -i -w 'HTTP %{http_version} %{http_code} d:%{size_download} u:%{size_upload} %header{x-request-id} %header{x-influxdb-error}' -sX POST 'http://localhost:8186/write?db=psrecord' --data-binary @virtualmem.txt
    ...
    if [[ "$count_cpu" -gt "$SPLITSIZE" ]]; then
      ...
      if [[ "$insert" = 'auto' ]]; then
        ...
        curl -i -w 'HTTP %{http_version} %{http_code} d:%{size_download} u:%{size_upload} %header{x-request-id} %header{x-influxdb-error}' -sX POST http://localhost:8186/query --data-urlencode "q=CREATE DATABASE psrecord" | awk '{print "     " $0}' | tail -1
      fi
      find . -type f -name "*-split-*" | sort | while read f; do
        ...
        if [[ "$insert" = 'auto' ]]; then
          ...
          # automatically run curl batch line insertions into InfluxDB database
          curl -i -w 'HTTP %{http_version} %{http_code} d:%{size_download} u:%{size_upload} %header{x-request-id} %header{x-influxdb-error}' -sX POST "http://localhost:8186/write?db=psrecord" --data-binary @${WORKDIR}/$fn | awk '{print "     " $0}' | tail -1
        fi
      done
    else
      echo
      ...
      if [[ "$insert" = 'auto' ]]; then
        # echo "     #  auto insert data into InfluxDB database: psrecord..."
        # automatically run curl batch line insertions into InfluxDB database
        curl -i -w 'HTTP %{http_version} %{http_code} d:%{size_download} u:%{size_upload} %header{x-request-id} %header{x-influxdb-error}' -sX POST "http://localhost:8186/write?db=psrecord" --data-binary @${WORKDIR}/cpuload.txt | awk '{print "     " $0}' | tail -1
      fi
      echo "     curl -i -sX POST 'http://localhost:8186/write?db=psrecord' --data-binary @${WORKDIR}/realmem.txt"
      if [[ "$insert" = 'auto' ]]; then
        # echo "     #  auto insert data into InfluxDB database: psrecord..."
        # automatically run curl batch line insertions into InfluxDB database
        curl -i -w 'HTTP %{http_version} %{http_code} d:%{size_download} u:%{size_upload} %header{x-request-id} %header{x-influxdb-error}' -sX POST "http://localhost:8186/write?db=psrecord" --data-binary @${WORKDIR}/realmem.txt | awk '{print "     " $0}' | tail -1
      fi
      echo "     curl -i -sX POST 'http://localhost:8186/write?db=psrecord' --data-binary @${WORKDIR}/virtualmem.txt"
      if [[ "$insert" = 'auto' ]]; then
        # echo "     #  auto insert data into InfluxDB database: psrecord..."
        # automatically run curl batch line insertions into InfluxDB database
        curl -i -w 'HTTP %{http_version} %{http_code} d:%{size_download} u:%{size_upload} %header{x-request-id} %header{x-influxdb-error}' -sX POST "http://localhost:8186/write?db=psrecord" --data-binary @${WORKDIR}/virtualmem.txt | awk '{print "     " $0}' | tail -1
      fi
      echo
